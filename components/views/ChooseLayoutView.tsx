...
import { ArrowLeft, Check, Grid, Sparkles, Layers, CheckCircle2 } from 'lucide-react';
import { FrameTemplate, LAYOUT_PRESETS, LayoutPreset } from '../../types/photobooth';
...
const activeLayout = LAYOUT_PRESETS.find(l => l.id === selectedLayoutId) || LAYOUT_PRESETS[0];

const layoutGroups = [
{ title: "1 POSE ARCHIVES", layouts: LAYOUT_PRESETS.filter(l => l.poseCount === 1) },
{ title: "2 POSES DUET", layouts: LAYOUT_PRESETS.filter(l => l.poseCount === 2) },
{ title: "3 POSES TRIO", layouts: LAYOUT_PRESETS.filter(l => l.poseCount === 3) },
{ title: "4 POSES QUAD", layouts: LAYOUT_PRESETS.filter(l => l.poseCount === 4) }
];
...
>
          ...
          {["All", "Football", "Retro", "Aesthetic"].map((cat) => {
            ...
            return (
              ...
              >
                ...
                <span className={`relative z-10 transition-colors duration-200 ${isActive ? 'text-white font-bold' : 'text-gray-400 hover:text-white'}`}>
                  {cat}
                </span>
              </button>
            );
              ...
              .map((frame) => {
                ...
                return (
                  <motion.button
                    ...
                    key={frame.id}
                    type="button"
                    onClick={() => { playSound('click'); setSelectedFrame(frame); }}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between w-full relative z-10">
                      ...
                      <span className={`font-pixel text-[6px] px-1.5 py-0.5 rounded uppercase tracking-wider ${getRarityStyles(frame.rarity)}`}>
                        {frame.rarity}
                      </span>
                    </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-5">
          ...
          <LayoutGroup id="pose-filters">
            <div className="flex bg-zinc-950 p-1.5 rounded-lg border border-white/5 gap-1.5 self-start md:self-auto shadow-inner">
              {["All", "1 Pose", "2 Pose", "3 Pose", "4 Pose"].map((group) => {
                const isActive = poseGroupFilter === group;
                return (
                  <button
                    key={group}
                    type="button"
                    onClick={() => { playSound('click'); setPoseGroupFilter(group); }}
                  ...
                  >
                    ...
                    <span className={`relative z-10 ${isActive ? 'text-[#FF9A9A]' : 'text-gray-500 hover:text-white'}`}>
                      {group}
                    </span>
                  </button>
                );
        ...
        <div className="flex flex-col gap-6 max-h-[480px] overflow-y-auto pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {layoutGroups
            .filter(group => {
              ...
              return group.layouts.length > 0 && group.layouts[0].poseCount === filterVal;
            })
            .map((group) => (
              <motion.div 
                layout="position"
                  ...
                  {group.layouts.map((layout) => {
                    ...
                    return (
                      <motion.div
                        ...
                        style={{ 
                          perspective: 1000,
                          borderColor: isSelected ? selectedFrame.borderColor : 'rgba(255,255,255,0.05)'
                        }}
                      >
                        ...
                        {isSelected && (
                          <div 
                            ...
                            style={{ borderColor: selectedFrame.borderColor }}
                          />
                        )}
                        ...
                        <div className="w-full text-center mt-3 relative z-10">
                          <div className="flex items-center justify-center gap-1.5">
                            <span className="font-display font-black text-xs text-white uppercase tracking-tight">{layout.name}</span>
                            {isSelected && (
                              <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }}>
                                <Check size={11} style={{ color: selectedFrame.borderColor }} />
                              </motion.span>
                            )}
                          </div>
    ...
    <div className="lg:col-span-4 lg:sticky lg:top-6 lg:self-start flex flex-col gap-4 bg-[#111111] border-2 border-white/10 p-4 md:p-5 rounded-2xl relative overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
      ...
      <div className="bg-white/5 border border-white/10 p-3 rounded-xl flex flex-col gap-1.5 text-[10px] font-mono shadow-inner relative z-10">
        <div className="flex justify-between items-center border-b border-white/5 pb-1">
          ...
          <span className="text-[#FF9A9A] font-bold uppercase">{activeLayout.name}</span>
        </div>
        <div className="flex justify-between items-center border-b border-white/5 pb-1">
          <span className="text-gray-400">Aspect Sheet:</span>
          <span className="text-white">{activeLayout.size === "2x6" ? "Stripe (2x6)" : "Postcard (4x6)"}</span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-gray-400">Pose Needed:</span>
          <span className="text-white font-bold">{activeLayout.poseCount} snapshots</span>
        </div>
      </div>

      {/* Confirm Button with Hover Pulse Effect */}
      <motion.button
        whileHover={{ scale: 1.03, boxShadow: `0 0 20px ${selectedFrame.borderColor}80` }}
        whileTap={{ scale: 0.97 }}
        type="button"
        onClick={() => { playSound('complete'); setPage(4); }}
  ...
  <div className="mt-4 pt-4 border-t border-white/5">
    <button
      type="button"
      onClick={() => { playSound('click'); setPage(1); }}
